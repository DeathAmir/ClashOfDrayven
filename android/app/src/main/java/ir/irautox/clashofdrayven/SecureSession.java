...
final class SecureSession {
    ...
    private static final String KEY_ALIAS="IrAutoX.ClashOfDrayven.Session.v1";
    private static final String TOKEN="token_v1";
    ...
    String load(){
        try{
            String packed=prefs.getString(TOKEN,null);if(packed==null||packed.isEmpty())return null;
        ...
        }catch(Exception ex){clear();return null;}
    }
    ...
    void save(String token){
        ...
        try{
            ...
            byte[] data=c.doFinal(token.getBytes(StandardCharsets.UTF_8));
            String packed=Base64.encodeToString(c.getIV(),Base64.NO_WRAP)+":"+Base64.encodeToString(data,Base64.NO_WRAP);
            prefs.edit().putString(TOKEN,packed).apply();
        }catch(Exception ex){throw new IllegalStateException("Secure session storage failed",ex);}
    }
    ...
    private static SecretKey key()throws Exception{
        ...
        java.security.Key existing=ks.getKey(KEY_ALIAS,null);if(existing instanceof SecretKey)return (SecretKey)existing;
