...
final class ApiClient {
    static final String BASE="http://irautox.ir:8456";
    private final SharedPreferences prefs;
    private String token;
    ApiClient(Context c){prefs=c.getSharedPreferences("drayven_session",Context.MODE_PRIVATE);token=prefs.getString("token",null);}
    String token(){return token;}
    void clear(){token=null;prefs.edit().remove("token").apply();}
    ...
    private void acceptToken(JSONObject r)throws JSONException{token=r.getString("token");prefs.edit().putString("token",token).apply();}
    JSONObject request(String method,String path,JSONObject body)throws Exception{
        HttpURLConnection c=(HttpURLConnection)new URL(BASE+path).openConnection();c.setRequestMethod(method);c.setConnectTimeout(12000);c.setReadTimeout(12000);c.setRequestProperty("Accept","application/json");c.setRequestProperty("User-Agent","ClashOfDrayven/3 Android");
    ...
    private static String friendly(String e){switch(e){case"invalid_credentials":return"Wrong username/email or password.";case"username_or_email_exists":return"Username or email already exists.";case"unauthorized":return"Session expired.";case"password_8_128":return"Password must be at least 8 characters.";default:return e.replace('_',' ');}}
}
