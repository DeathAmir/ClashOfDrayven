...
final class PackManager {
    ...
    static PackManager prepare(Context context)throws Exception{
        File root=new File(context.getFilesDir(),"drayven-assets-v6");
        File stamp=new File(root,".stamp");
        PackageInfo info=context.getPackageManager().getPackageInfo(context.getPackageName(),0);
        String wanted="v6:"+info.lastUpdateTime;
    ...
    Typeface font(){
        ...
        try{File f=exact("Fonts/DRY.ttf");font=f!=null?Typeface.createFromFile(f):Typeface.DEFAULT_BOLD;}
        catch(Exception ex){font=Typeface.DEFAULT_BOLD;}
        return font;
    }

    File exact(String rel){
        if(rel==null)return null;
        try{
            File f=new File(root,rel.replace('/',File.separatorChar)).getCanonicalFile();
            String base=root.getCanonicalPath()+File.separator;
            if(!f.getPath().startsWith(base)||!f.isFile())return null;
            return f;
        }catch(Exception ex){return null;}
    }

    Bitmap bitmap(String rel){
        if(bitmapCache.containsKey(rel))return bitmapCache.get(rel);
        File f=exact(rel);Bitmap b=f==null?null:BitmapFactory.decodeFile(f.getAbsolutePath());
        bitmapCache.put(rel,b);return b;
    }
    ...
    private static void unpack(Context context,String assetName,File root)throws Exception{
        ...
        try(InputStream base=new BufferedInputStream(context.getAssets().open(assetName))){
            byte[] magic=readN(base,MAGIC.length);if(!Arrays.equals(magic,MAGIC))throw new IOException("bad CLDRYPK magic: "+assetName);
            int version=readI32(base);if(version!=1)throw new IOException("unsupported CLDRYPK version");
            long rawLength=readI64(base);int hashLen=readI32(base);byte[] expected=readN(base,hashLen);byte[] compressed=readAll(base);
            if(!inflate(compressed,raw,true)&&!inflate(compressed,raw,false))throw new IOException("CLDRYPK deflate decode failed");
        ...
        try(InputStream in=new BufferedInputStream(new FileInputStream(raw),128*1024)){
            int count=readI32(in);String rootPath=root.getCanonicalPath()+File.separator;
            for(int i=0;i<count;i++){
                String rel=readDotNetString(in).replace('/',File.separatorChar);long len=readI64(in);byte[] expected=readN(in,readI32(in));
                File dest=new File(root,rel).getCanonicalFile();if(!dest.getPath().startsWith(rootPath))throw new IOException("unsafe pack path");
                File parent=dest.getParentFile();if(parent!=null)parent.mkdirs();MessageDigest md=MessageDigest.getInstance("SHA-256");
                try(OutputStream out=new BufferedOutputStream(new FileOutputStream(dest))){copy(in,out,len,md);}if(!Arrays.equals(expected,md.digest()))throw new IOException("file hash mismatch: "+rel);
            }
        }finally{raw.delete();}
    }
