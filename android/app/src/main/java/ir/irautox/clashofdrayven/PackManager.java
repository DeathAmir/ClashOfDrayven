...
final class PackManager {
    ...
    private final File root;private final HashMap<String,Bitmap> bitmapCache=new HashMap<>();private Typeface font;
    private PackManager(File root){this.root=root;}

    static PackManager prepare(Context context)throws Exception{
        File root=new File(context.getFilesDir(),"drayven-assets-v6");File stamp=new File(root,".stamp");PackageInfo info=context.getPackageManager().getPackageInfo(context.getPackageName(),0);String wanted="v6:"+info.lastUpdateTime;
        if(!(stamp.isFile()&&wanted.equals(readText(stamp)))){delete(root);if(!root.mkdirs()&&!root.isDirectory())throw new IOException("cannot create asset cache");for(int i=1;i<=20;i++)unpack(context,"packs/CLDRYPK"+i,root);writeText(stamp,wanted);}
        PackManager pm=new PackManager(root);try{pm.font=Typeface.createFromAsset(context.getAssets(),"fonts/vazir.ttf");}catch(Exception ignored){}return pm;
    }
    File root(){return root;}
    Typeface font(){if(font!=null)return font;try{File f=exact("Fonts/DRY.ttf");font=f!=null?Typeface.createFromFile(f):Typeface.DEFAULT_BOLD;}catch(Exception ex){font=Typeface.DEFAULT_BOLD;}return font;}
